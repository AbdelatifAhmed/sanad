...
class DynamicGatewayLLM {
  constructor() {
    ...
    this.globalTimeoutMs = 12000; // 12 seconds maximum total per request
  }
  ...
  async invoke(messages, options = {}) {
    ...
    // تحويل الـ messages لصيغة صريحة تناسب الـ API
    ...
    // دمج الـ System prompt مع الـ Messages ليتناسب مع هيكلية Pollinations المفتوحة
    const finalMessages = [
    ...
    for (const modelId of this.models) {
      ...
      try {
        ...
        // Fast Fail Circuit Breaker for Gateway down or 403 Forbidden
        if (statusCode === 403 || statusCode === 401) {
          throw new Error(`AI Gateway Authentication/Access Error: ${errorMsg}`);
        }
        
        // Network drop or ENOTFOUND (Gateway completely unreachable)
