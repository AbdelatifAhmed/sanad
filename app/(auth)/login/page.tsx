...
function LoginContent() {
  ...
  const t = {
    ...
    passwordRequired: isAr ? "كلمة المرور مطلوبة." : "Password is required.",
    passwordMinLength: isAr ? "يجب أن تتكون كلمة المرور من 8 أحرف على الأقل." : "Password must be at least 8 characters.",
    invalidCredentials: isAr ? "بيانات الدخول غير صحيحة. يرجى المحاولة مرة أخرى." : "Invalid credentials. Please try again.",
    welcomeBack: isAr ? "مرحباً بك مجدداً!" : "Welcome Back!",
    signingIn: isAr ? "جاري تسجيل الدخول، يرجى الانتظار..." : "Signing you in, please wait...",
    welcomeTitle: isAr ? "مرحباً بك مجدداً" : "Welcome back",
    enterDetails: isAr ? "الرجاء إدخال بياناتك لتسجيل الدخول." : "Please enter your details to sign in.",
    ...
    signingInBtn: isAr ? "جاري تسجيل الدخول..." : "Signing In...",
    orContinueWith: isAr ? "أو تابع باستخدام" : "Or continue with",
    dontHaveAccount: isAr ? "ليس لديك حساب؟" : "Don't have an account?",
    signUp: isAr ? "إنشاء حساب" : "Sign up",
    dignifiedCareTitle: isAr ? "رعاية كريمة،\nقريبة من منزلك." : "Dignified care,\ncloser to home.",
    dignifiedCareDesc: isAr
      ? "نربط العائلات بمرافقين رحماء وموثوقين لرعاية كبار السن. استمتع براحة البال مع سند."
  ...
  return (
    <div className="relative min-h-screen flex flex-col md:flex-row font-body bg-[#fcf9f6]" dir={isAr ? "rtl" : "ltr"}>
      ...
      <section className="relative w-full md:w-[43%] min-h-screen flex flex-col justify-between p-8 md:p-10 overflow-hidden">
        ...
        <div className={`z-10 relative max-w-xl space-y-4 mt-auto ${isAr ? "text-right" : "text-left"}`}>
          <h1 className="font-display text-3xl md:text-4xl lg:text-4.5xl font-bold text-primary leading-[1.2] tracking-tight whitespace-pre-line">
            {t.dignifiedCareTitle}
          </h1>
          ...
          {success ? (
            ...
            <div className="bg-white rounded-3xl border border-sand-high p-8 md:p-10 custom-shadow space-y-8">
              <div className="flex flex-col items-center text-center">
                <h2 className="font-display text-2.5xl font-bold text-primary mb-1">{t.welcomeTitle}</h2>
                <p className="text-gray-500 text-sm font-medium">{t.enterDetails}</p>
              </div>

              {serverError && (
                <div className={`p-4 bg-red-50 rounded-md text-red-700 text-sm ${isAr ? "border-r-4 border-red-500" : "border-l-4 border-red-500"}`}>
                  {serverError}
                </div>
              )}
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="border-t border-sand-high pt-6 space-y-5">
                  ...
                  <div className="space-y-1.5">
                    <label className={`block text-sm font-bold text-gray-700 ${isAr ? "text-right" : "text-left"}`}>
                      {t.emailLabel}
                    </label>
                    <div className="relative">
                      <Mail className={`absolute ${isAr ? "right-4" : "left-4"} top-1/2 -translate-y-1/2 text-gray-400 w-5 h-5`} />
                      <input
                        type="email"
                        name="email"
                    ...
                    {errors.email && <p className={`text-red-500 text-xs font-semibold ${isAr ? "text-right" : "text-left"}`}>{errors.email}</p>}
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <Lock className={`absolute ${isAr ? "right-4" : "left-4"} top-1/2 -translate-y-1/2 text-gray-400 w-5 h-5`} />
                      <input
                        type={showPassword ? "text" : "password"}
                        name="password"
                        value={formData.password}
                        onChange={handleInputChange}
                        className={`w-full py-3.5 bg-white border border-outline-variant rounded-xl focus:ring-2 focus:ring-primary focus:border-primary transition-all placeholder:text-gray-400 text-sm ${isAr ? "pr-12 pl-12 text-right" : "pl-12 pr-12 text-left"}`}
                        placeholder="••••••••"
                      />
                      <button
                        type="button"
                    ...
                    {errors.password && <p className={`text-red-500 text-xs font-semibold ${isAr ? "text-right" : "text-left"}`}>{errors.password}</p>}
                  </div>
                </div>

                {/* Remember Me */}
                <div className={`flex items-center ${isAr ? "justify-start" : ""}`}>
                  <input
                    id="remember-me"
                    type="checkbox"
                    checked={rememberMe}
                  ...
                  />
                  <label htmlFor="remember-me" className={`${isAr ? "mr-2" : "ml-2"} block text-sm text-gray-600 cursor-pointer select-none`}>
                    {t.rememberMe}
                  </label>
                </div>
                ...
                >
                  <span>{loading ? t.signingInBtn : t.signIn}</span>
                ...
                <div className="relative my-6">
                  ...
                  <div className="relative flex justify-center text-xs uppercase">
                    <span className="bg-white px-2 text-gray-400 font-bold">{t.orContinueWith}</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <button
                    ...
                    className="flex justify-center items-center h-12 border border-outline-variant rounded-xl bg-white hover:bg-sand-low transition-colors duration-200 text-gray-700 font-bold text-sm cursor-pointer"
                  >
                    <svg className={`w-5 h-5 text-black ${isAr ? "ml-2" : "mr-2"}`} fill="currentColor" viewBox="0 0 24 24">
                ...
                <p className="text-center text-sm text-gray-500 pt-2 font-medium">
                  <span className={`${isAr ? "ml-1" : "mr-1"}`}>{t.dontHaveAccount}</span>
