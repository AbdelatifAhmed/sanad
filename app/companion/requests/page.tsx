...
export default function CompanionRequests() {
    ...
    try {
      ...
      const mapped: ServiceRequest[] = bookings.map((b: any) => {
        ...
        return {
          ...
          title: type === "Medical" ? "Medical Assistance" : type === "Personal Care" ? "Personal Care & Hygiene" : "Companion Care",
          location: b.familyId?.location?.readableAddress || b.familyId?.location?.city || "Client Residence",
          ...
          description: b.notes || "No additional instructions provided by the family.",
  ...
  const handleAccept = async (id: string) => {
    ...
    try {
      ...
      setAnimatingId(null);
      const msg = err.response?.data?.error || err.response?.data?.message || "Failed to accept booking. Please try again.";
      setToast({
        message: msg,
        type: "info",
      });
  ...
  const handleReject = async (id: string) => {
    ...
    try {
      ...
      setAnimatingId(null);
      const msg = err.response?.data?.error || err.response?.data?.message || "Failed to decline booking. Please try again.";
      setToast({
        message: msg,
        type: "info",
      });
    ...
    <div className="relative font-stitch-body select-none">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <div className="bg-white px-4 py-2.5 rounded-2xl shadow-soft flex items-center gap-2 border border-stitch-outline/10 text-stitch-on-surface-variant">
            <span className="material-symbols-outlined text-stitch-primary text-xl">filter_list</span>
            <select
              value={urgencyFilter}
          ...
          {/* Distance */}
          <div className="bg-white px-4 py-2.5 rounded-2xl shadow-soft flex items-center gap-2 border border-stitch-outline/10 text-stitch-on-surface-variant">
            <span className="material-symbols-outlined text-stitch-primary text-xl">near_me</span>
            <select
          ...
          {/* Category */}
          <div className="bg-white px-4 py-2.5 rounded-2xl shadow-soft flex items-center gap-2 border border-stitch-outline/10 text-stitch-on-surface-variant">
            <span className="material-symbols-outlined text-stitch-primary text-xl">category</span>
            <select
              value={typeFilter}
      ...
      {loading && (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          {[1, 2].map((n) => (
            <div key={n} className="bg-white p-6 rounded-3xl shadow-soft border border-slate-100 flex flex-col gap-5 animate-pulse">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
              ...
              >
                <div className="flex justify-between items-start gap-3">
                  <div className="flex gap-4">
                    {/* Category icon */}
                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 ${getIconClass(req.type)}`}>
                      <span className="material-symbols-outlined text-3xl">{getIcon(req.type)}</span>
                    </div>
                    <div>
                      {/* Urgency Badge */}
                      <span
                      ...
                      <p className="text-stitch-on-surface-variant/80 flex items-center gap-1 text-xs mt-1">
                        <span className="material-symbols-outlined text-sm text-stitch-primary">location_on</span>
                        {req.location} • <span className="font-semibold text-stitch-primary">{req.distance} km away</span>
                      </p>
                    </div>
                  </div>
                  {/* Rate and Time */}
                ...
                <div className="flex gap-6 bg-slate-50/70 border border-slate-100 p-4 rounded-2xl text-xs">
                  <div className="flex flex-col">
                    <span className="text-stitch-on-surface-variant/50 font-bold uppercase tracking-wider text-[9px] mb-0.5">DATE</span>
                    <span className="font-bold text-[#2b2b2b]">{req.date}</span>
                  </div>
                  <div className="w-px bg-slate-200" />
                  <div className="flex flex-col">
                    <span className="text-stitch-on-surface-variant/50 font-bold uppercase tracking-wider text-[9px] mb-0.5">TIME</span>
          ...
          {filteredRequests.length === 0 && (
            <div className="col-span-1 xl:col-span-2 border-2 border-dashed border-stitch-outline/20 p-12 rounded-3xl flex flex-col items-center justify-center text-center bg-white/40">
              <span className="material-symbols-outlined text-stitch-outline/60 text-5xl mb-3">cloud_off</span>
      ...
      <footer className="w-full py-8 mt-16 border-t border-stitch-outline/10 flex flex-col sm:flex-row justify-between items-center gap-4 text-xs text-stitch-on-surface-variant/60">
        <div>
          <h4 className="font-stitch-display font-bold text-[#012d1d] text-sm mb-1">Dignified Care</h4>
          <p>© 2026 Dignified Care. All rights reserved. Compassionate elder support.</p>
        </div>
        <div className="flex flex-wrap gap-4 justify-center sm:justify-end items-center">
          <a href="#" className="hover:text-stitch-primary transition-colors">Privacy Policy</a>
          <a href="#" className="hover:text-stitch-primary transition-colors">Terms of Service</a>
          <a href="#" className="hover:text-stitch-primary transition-colors">Cookie Policy</a>
        </div>
      </footer>
      ...
      <div className="fixed bottom-6 right-6 md:bottom-8 md:right-8 bg-white p-4 rounded-2xl shadow-premium border border-stitch-primary/10 z-40 hidden lg:flex flex-col gap-2 min-w-[160px] animate-fade-in">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-[10px] text-stitch-on-surface-variant uppercase tracking-wider">Live Status</span>
        </div>
        <div className="flex gap-6 mt-1">
          <div>
            <p className="text-[9px] uppercase tracking-wider text-stitch-on-surface-variant/50">NEW JOBS</p>
            <p className="text-xl font-bold text-stitch-primary">{requests.length}</p>
          </div>
          <div className="border-l border-stitch-outline/20 h-8"></div>
          <div>
            <p className="text-[9px] uppercase tracking-wider text-stitch-on-surface-variant/50">BOOKED</p>
            <p className="text-xl font-bold text-stitch-on-surface">{activeBookingsCount}</p>
          </div>
        </div>
      </div>
